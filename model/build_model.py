...
class RecommendationEngine:
    ...
    def predict(self, user_inputs):
        """
        Fast prediction using pre-built index
        Args:
            user_inputs: dict with keys 'gender', 'articleType', 'season', 'usage'
        Returns:
            list of recommended product IDs (strings)
        """
        if self.model is None or self.index is None:
            raise RuntimeError("Model not loaded. Call load_model_and_index() first.")

        # Create query from user inputs
        user_query = {
        ...
        # Get recommendations (fast - just a lookup)
        _, top_indices = self.index(user_query, k=5)
        # Map indices to IDs
        top_ids = tf.gather(self.all_identifiers, top_indices)
        # Convert to Python list of strings
        recommendations = [id.decode('utf-8') for id in top_ids[0].numpy()]
